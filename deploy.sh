...
# Judge0's pandas/numpy image is published to GHCR as a private package (see
# agents/codeforge_agent/judge0/docker-compose.yml) -- changing package
# visibility to public requires GitHub *organization owner* rank, which
# isn't something every contributor who can deploy necessarily has, so this
# authenticates instead. GHCR_PULL_TOKEN is a classic PAT with only the
# read:packages scope, generated by anyone whose account can already view
# the package (the same access this deploy already implicitly relies on),
# stored as this repo's GHCR_PULL_TOKEN secret. Skips silently if unset, so
# deploy.sh keeps working for manual/local runs and before the secret exists.
