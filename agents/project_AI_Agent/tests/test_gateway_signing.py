"""Gateway signature verification (capstone_project_agent). Kept in step with orchestrator/tests/test_agent_signing.py."""
from fastapi import FastAPI, Request
from fastapi.testclient import TestClient
from httpx import ASGITransport, AsyncClient

from app.integration import agent_signing as signing

AGENT = "capstone_project_agent"
INTERNAL_HITS = []
...
def make_client():
    app = FastAPI()
    app.add_middleware(signing.GatewaySignatureMiddleware, agent_name=AGENT)

    @app.post("/api/invoke")
    async def invoke(request: Request):
        return {"admin": request.headers.get("x-digidara-is-admin"), "size": len(await request.body()), "body": await request.json()}
...
def test_warn_mode_serves_unsigned_requests_but_logs_them(caplog):
    with caplog.at_level("WARNING", logger="digidara.signing"):
        response = make_client().post("/api/invoke", json={"action": "x"}, headers={"x-digidara-is-admin": "true"})
...
def test_enforce_accepts_a_correctly_signed_request_and_preserves_the_body(monkeypatch):
    monkeypatch.setenv("AGENT_SIGNATURE_MODE", "enforce")
    body = json.dumps({"action": "get_profile", "payload": {"pad": "x" * 200_000}}).encode()
    headers = sign(body, user_id="u1", is_admin="false") | {"content-type": "application/json"}
    response = make_client().post("/api/invoke", content=body, headers=headers)
    assert response.status_code == 200
    assert response.json()["admin"] == "false" and response.json()["size"] == len(body)
...
def test_enforce_rejects_a_signed_request_whose_admin_flag_was_flipped(monkeypatch):
    ...
    assert make_client().post("/api/invoke", content=body, headers=headers).status_code == 401
...
def test_enforce_rejects_a_replayed_request(monkeypatch):
    ...
    assert client.post("/api/invoke", content=body, headers=headers).status_code == 200
    assert client.post("/api/invoke", content=body, headers=headers).status_code == 401


def test_enforce_still_allows_the_docker_healthcheck_and_health_route(monkeypatch):
    monkeypatch.setenv("AGENT_SIGNATURE_MODE", "enforce")
    client = make_client()
    assert client.post("/api/invoke", content=b'{"action": "health"}', headers={"content-type": "application/json"}).status_code == 200
