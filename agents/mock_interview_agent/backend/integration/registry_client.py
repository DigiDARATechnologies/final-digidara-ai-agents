"""Self-register this Flask service with the DigiDARA orchestrator.

Copied from agents/aptitude_agent/backend/app/integration/registry_client.py
(the reference implementation for a Flask agent) with only the module root
and logger name adjusted -- keep the two in sync if the shared HMAC
registration contract ever changes.
"""
...
logger = logging.getLogger("mock_interview.integration.registry")
...
def _register() -> None:
    ...
    try:
        ...
        logger.warning("Mock interview registry registration failed", exc_info=True)
...
def _heartbeat() -> None:
    ...
    try:
        ...
        logger.warning("Mock interview registry heartbeat failed", exc_info=True)
...
def start() -> None:
    ...
    _started = True
    _register()
    _thread = threading.Thread(target=_loop, name="mock-interview-registry-heartbeat", daemon=True)
    _thread.start()
