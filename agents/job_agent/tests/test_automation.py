...
from job_agent import automation, worker
...
class WorkerHealthTests(unittest.TestCase):
    def test_worker_heartbeat_reports_fresh_then_stale(self):
        with tempfile.TemporaryDirectory() as directory:
            heartbeat = os.path.join(directory, "worker-heartbeat")
            with patch.object(worker, "WORKER_HEARTBEAT_FILE", heartbeat), patch.object(
                worker, "WORKER_HEARTBEAT_STALE_SECONDS", 120
            ):
                worker._touch_heartbeat()
                self.assertTrue(worker._heartbeat_is_fresh())
                stale_time = time.time() - 121
                os.utime(heartbeat, (stale_time, stale_time))
                self.assertFalse(worker._heartbeat_is_fresh())
