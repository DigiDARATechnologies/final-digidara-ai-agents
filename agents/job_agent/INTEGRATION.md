...
- LinkedIn/Naukri/Indeed/Glassdoor/Foundit providers — `providers/apify.py`
  is a working Apify REST client, but `normalize_job()` needs one mapping
  per platform's actual Apify actor output shape, which hasn't been
  provided yet.
