...
lifespan hook. `start()` is idempotent, guarded against the double import a
dev auto-reloader (`flask run --debug`, `FLASK_ENV=development`) triggers.
"""
...
logger = logging.getLogger("communication_agent.integration.registry_client")
...
def _register():
    ...
    try:
        ...
        logger.warning("communication_agent registration failed; will retry on next heartbeat", exc_info=True)
...
def _heartbeat():
    ...
    try:
        ...
        logger.warning("communication_agent heartbeat failed", exc_info=True)
