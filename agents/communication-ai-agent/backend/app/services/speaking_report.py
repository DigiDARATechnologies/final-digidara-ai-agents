"""Generate learner-facing Speaking Practice Assessment PDFs."""
...
def _draw_logo_and_footer(canvas, doc):
    ...
    footer_text = "DigiDARA AI Agents Platform  |  Communication Coach Speaking Practice Report"
...
def generate_speaking_report_pdf(session, learner_name="Learner"):
    """Compile a professional, beautiful PDF report for a completed speaking session."""
    buffer = BytesIO()
    ...
    # Title & Subtitle
    story.append(Paragraph("Speaking Practice Report", title_style))
    ...
    meta_data = [
        ...
        [
            Paragraph(f"<b>Answered Turns:</b> {session.answered_turns or len(session.turns)}", body_style),
    ...
    fluency_val = round(float(session.fluency_score), 1) if getattr(session, "fluency_score", None) is not None else overall_val
    grammar_val = round(float(session.grammar_score), 1) if getattr(session, "grammar_score", None) is not None else overall_val
    confidence_val = round(float(session.confidence_score), 1) if getattr(session, "confidence_score", None) is not None else overall_val

    score_data = [
        [
            Paragraph(f"{overall_val} <font size=10>/ 10</font>", score_big),
            Paragraph(f"{fluency_val} <font size=10>/ 10</font>", score_big),
            Paragraph(f"{grammar_val} <font size=10>/ 10</font>", score_big),
            Paragraph(f"{confidence_val} <font size=10>/ 10</font>", score_big),
        ],
        [
            Paragraph("OVERALL SCORE", score_label),
            Paragraph("FLUENCY", score_label),
            Paragraph("GRAMMAR", score_label),
            Paragraph("CONFIDENCE", score_label),
        ],
    ]
    ...
    improvements = []
    if getattr(session, "areas_to_improve_json", None):
        try:
            improvements = json.loads(session.areas_to_improve_json)
        except Exception:
            pass
    if not improvements:
    ...
    story.append(Paragraph("Turn-by-Turn Conversation Transcript & Corrections", section_h1))
    
    answered_turns = [t for t in session.turns if t.user_answer]
    if not answered_turns:
        story.append(Paragraph("No spoken turns were completed in this session.", muted_body))
    else:
        for idx, turn in enumerate(answered_turns, start=1):
            feedback_text = getattr(turn, "reaction", "") or ""
            correction = getattr(turn, "corrected_answer", None)
            
            turn_elements = [
                Paragraph(f"<b>Turn {idx} - Question:</b> {_plain(turn.ai_question)}", body_style),
                Spacer(1, 2),
                Paragraph(f"<b>You Said:</b> <i>\"{_plain(turn.user_answer)}\"</i>", ParagraphStyle("Ans", parent=body_style, textColor=colors.HexColor("#2B6CB0"))),
            ]
            if feedback_text:
                turn_elements.append(Spacer(1, 2))
                turn_elements.append(Paragraph(f"<b>Coach Reaction:</b> {_plain(feedback_text)}", muted_body))
            if correction:
                turn_elements.append(Spacer(1, 2))
                turn_elements.append(Paragraph(f"<b>Suggested Correction:</b> <font color='#167A50'>{_plain(correction)}</font>", body_style))
