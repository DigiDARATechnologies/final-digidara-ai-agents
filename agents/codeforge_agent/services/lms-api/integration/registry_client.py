...
lifespan hook. This app always runs via `waitress.serve(...)` in run.py,
never a dev-server reloader, so there's no double-import/double-registration
...
logger = logging.getLogger("codeforge.integration.registry_client")
...
def _register():
    ...
    try:
        ...
        logger.warning("codeforge_agent registration failed; will retry on next heartbeat", exc_info=True)
...
def _heartbeat():
    ...
    try:
        ...
        logger.warning("codeforge_agent heartbeat failed", exc_info=True)
