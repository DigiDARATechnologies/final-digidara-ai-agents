...
export default function ClassicClearTemplate({ resume }) {
  const normalized = normalizeResumeForTemplate(resume);
  const info = normalized.personal_info ?? {};
  const role = normalized.target_role;
  return (
    <div className="resume-page template-classic-clear layout-single-column">
      <ResumeHeader info={info} role={role} />
      <div className="resume-content">
        <main className="resume-main">
          {renderSections(normalized, REQUIRED_SECTION_ORDER)}
        </main>
      </div>
    </div>
  );
