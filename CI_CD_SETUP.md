...
- Security: npm audit fails on high/critical findings; pip-audit checks exact installed dependencies exported from each built agent image and fails on any known vulnerability. Trivy scans all seven agent images and the frontend runtime image for high/critical OS-package vulnerabilities that have an available fix (`ignore-unfixed: true`) — the `python:3.12-slim` base left every agent with 50-90+ high/critical findings that Debian has not patched, which `ignore-unfixed: false` cannot ever pass regardless of application code; findings with a real fix still fail the build. `Dockerfile.frontend` runs `apk upgrade` on the `nginx:alpine` base at build time so fixed OS packages are actually applied. JSON reports upload even on failure. Two pip-audit findings are explicitly ignored via `--ignore-vuln` because pip-audit itself reports no available fix version for either: `PYSEC-2025-183` (PyJWT) and `PYSEC-2026-1325` (ecdsa, a transitive dependency). No other vulnerabilities are suppressed and no scan uses continue-on-error; re-check ignored findings periodically and remove the ignore once upstream ships a fix.
