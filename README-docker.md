...
- `docker/mysql-init/01-create-databases.sql` and `docker-compose.yml` also
  ...
  On an existing MySQL volume, initialization scripts do not run again;
  create `job_agent` once before deploying these two services, for example
  with `docker compose exec mysql mysql -uroot -p -e "CREATE DATABASE IF NOT EXISTS job_agent"`.
