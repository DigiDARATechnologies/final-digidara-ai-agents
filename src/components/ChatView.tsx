...
interface ChatViewProps {
   ...
   * everything after it, resuming the flow from that point ╬ô├ç├╢ see
  ...
  /** File input `accept` string ╬ô├ç├╢ varies per agent (Capstone wants
   ...
   * of the normal single-line input ╬ô├ç├╢ needed for pasting a full resume or a
   ...
   * any), and ╬ô├ç├╢ for agents that have one ╬ô├ç├╢ a quick per-agent option
  ...
  /** Extra panel rendered inside the latest agent message, above its text ╬ô├ç├╢
  ...
  /** Extra panel rendered above the message list ╬ô├ç├╢ used by the Aptitude
...
export default function ChatView({
  ...
  onSubmitCode,
  multilineMode = false,
  multilinePlaceholder = "Paste text here╬ô├ç┬¬",
  ...
  // background after Send unless explicitly stopped ╬ô├ç├╢ and a result that was
  ...
  async function handleCopy(index: number, text: string) {
    try {
      ...
      return; // Clipboard access denied/unavailable ╬ô├ç├╢ silently skip rather than show a false "Copied".
    }
  ...
  function handleSubmit(e: FormEvent) {
    ...
    if (!text) {
      setInputError(multilineMode ? "Please write your paragraph before submitting. Γëí╞Æ├┐├¿" : "Please enter a message before sending.");
      return;
    }
    setInputError("");
    voiceSessionRef.current += 1;
  ...
  // <input> ΓÇö a plain <input> can never hold a real newline at all, which is
  ...
  useEffect(() => {
    ...
    if ("speechSynthesis" in window && "SpeechSynthesisUtterance" in window) {
      const utterance = new SpeechSynthesisUtterance(activeSpeakingPrompt.replace(/[Γö¼Γòû╬ô├ç├╢]/g, " "));
      utterance.rate = 0.96;
      utterance.pitch = 1;
  ...
  return (
    <section className={`view view-chat active${immersiveSpeaking ? " immersive-speaking" : ""}`} id="view-chat">
      <div className="chat-header">
        ...
        {dailyChallengeStatus && (
          <button type="button" className={`daily-challenge-pill ${dailyChallengeStatus}`} onClick={onDailyChallenge}>
            <span>Γëí╞Æ├┤├á</span><span>Daily Challenge</span>
      ...
      <div className="chat-messages" ref={messagesRef}>
        {immersiveSpeaking && <div className="speaking-stage"><div className="speaking-stage-copy"><span>Speaking practice</span><h2>{activeSpeakingPrompt}</h2><p>{typing ? "Coach is preparing the next question╬ô├ç┬¬" : agentSpeaking ? "Coach is speaking╬ô├ç┬¬" : speech.listening ? "Listening automatically Γö¼Γòû sends after 3 seconds of silence" : "Starting conversation╬ô├ç┬¬"}</p></div><button type="button" aria-label={speech.listening ? "Stop listening" : "Start speaking"} className={`speaking-orb${speech.listening ? " listening" : ""}`} onClick={handleMicClick} /><button type="button" className="speaking-end" onClick={() => onChooseOption("end_session")}>End session</button></div>}
      ...
      {certificateExamInstructions && (
        <div className="certificate-instructions-backdrop" role="dialog" aria-modal="true" aria-labelledby="certificate-instructions-title">
          <section className="certificate-instructions-modal">
            <div className="certificate-instructions-icon">Γëí╞Æ├ä├┤</div>
      ...
      {codeMode ? (
        ...
        <form className="composer" onSubmit={handleSubmit}>
          ...
          <textarea
            ...
            rows={1}
            placeholder="Message DigiDARAΓÇª"
            value={input}
            disabled={composerDisabled}
            onChange={(e) => { setInput(e.target.value); if (inputError) setInputError(""); }}
            onKeyDown={(e) => {
