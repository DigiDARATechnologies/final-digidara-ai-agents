...
export default function SettingsModal({ open, user, chats, glowOn, onClose, onOpenChat, onGlowToggle, onClearHistory, onToast, onExportData, onDeleteAccount }: Props) {
  ...
  async function checkout(planId: string) {
    ...
    try {
      ...
      const checkoutInstance = new Razorpay({ key: order.key_id, amount: order.amount, currency: order.currency, name: "DigiDARA", description: `${order.name} plan`, order_id: order.order_id, prefill: { name: user.name, email: user.email, contact: user.mobile }, theme: { color: "#6d5bff" },
  ...
  async function topup(amountInr: number) {
    ...
    try {
      ...
      const Razorpay = (window as unknown as { Razorpay: RazorpayConstructor }).Razorpay;
      const checkoutInstance = new Razorpay({ key: order.key_id, amount: order.amount, currency: order.currency, name: "DigiDARA", description: `${order.name} top-up`, order_id: order.order_id, prefill: { name: user.name, email: user.email, contact: user.mobile }, theme: { color: "#6d5bff" },
